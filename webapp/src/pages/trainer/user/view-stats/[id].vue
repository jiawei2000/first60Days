    ...
    <div>
        ...
        <div class="d-flex align-center justify-space-between mb-4">
            ...
            <div class="d-flex align-center">
                <VBtnToggle v-model="timeFrame" divided color="primary" density="comfortable" class="mr-4">
                    <VBtn value="daily">Daily</VBtn>
                    <VBtn value="weekly">Weekly</VBtn>
                </VBtnToggle>

                <!-- 🔄 Toggle -->
                <VBtn variant="tonal" color="primary" size="small" @click="goToOtherPage">
                    🔄 Switch to Schedule
                </VBtn>
            </div>
        </div>
        ...
        <VCard>
            ...
            <VWindow v-model="activeTab" class="pa-4">
                ...
                <!-- 🧭 Shared Date Picker -->
                <div v-if="timeFrame === 'daily'" class="d-flex align-center justify-end mb-2"
                    style="position: relative; z-index: 10;">
                    <VMenu v-model="dateMenu.value" transition="scale-transition" location="bottom end" offset-y
                        max-width="290" min-width="auto" :close-on-content-click="false">
                        <template #activator="{ props }">
                            <VBtn v-bind="props" variant="flat" color="primary" size="small" rounded="lg" elevation="1">
                                {{ selectedDate ? formatLocalDate(selectedDate) : 'Select date' }}

                            </VBtn>
                        </template>

                        <VDatePicker v-model="selectedDate" color="primary" :max="new Date()" @update:modelValue="() => {
                ...
                <VWindowItem value="summary">
                    <VCardText>
                        ...
                        <div v-if="timeFrame === 'weekly'" class="mb-4" style="max-width: 200px;">
                            <VSelect v-model="selectedWeek" :items="weekOptions" item-title="title" item-value="value"
                                label="Select Week" density="comfortable" variant="outlined" hide-details />
                        </div>


                        <p v-if="timeFrame === 'weekly' && currentWeekLabel"
                            class="text-caption text-medium-emphasis mb-4">
                            Showing week: <strong>{{ currentWeekLabel }}</strong>
                        </p>

                        <VRow>
                            <VCol v-for="metric in metricOptions" :key="metric.value" cols="12" sm="6" md="4">
                                <VCard class="pa-4" elevation="1">
                                    <div class="d-flex align-center justify-space-between">
                                        ...
                                        <!-- STATUS CHIP -->
                                        <VChip :color="getThresholdStatus(metric.value).color" text-color="white" label
                                            size="small">
                ...
                <VWindowItem value="trends">
                    <VCardText>
                        ...
                        <!-- Metric Selector -->
                        <VSelect v-model="selectedMetric" :items="metricOptions" :item-title="item => `${item.title}`"
                            item-value="value" label="Select Metric" density="compact" hide-details variant="outlined"
                            class="mb-4" />
                        ...
                        <div class="d-flex align-center justify-space-between mb-6">
                            <div>
                                ...
                                <div v-if="percentChange !== null" :class="[
                                    ...
                                    <img :src="percentChange >= 0 ? upArrow : downArrow" alt="arrow"
                            ...
                            <div class="text-center">
                                <VCircularProgress :model-value="circleValue" color="primary" size="70" width="6" />
                        ...
                        <!-- LINE CHART -->
                        <ApexChart v-if="chartReady" type="line" height="250" :options="chartOptions"
                            :series="chartSeries" />
                    </VCardText>
                </VWindowItem>
...
async function fetchWeeklyStats() {
    try {
        ...
        weeklyStats.value = statsArr.map((s, index) => ({
            ...
            totalMilkVolume: s.totalMilkVolume,
